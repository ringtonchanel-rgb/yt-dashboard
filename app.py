# app.py — Dashboard (мульти-группы с управлением) + Group Analytics (Year Mix)
# Исправлено: st.rerun() вместо experimental_rerun, .empty как свойство, надёжное чтение/дедупликация файлов.
...
def detect_columns(df: pd.DataFrame): return {k: find_col(df, v) for k,v in MAP.items()}
...
def to_number(x):
    ...
    try:
        val=float(s)
        return val  # оставляем в «процентном» виде (5.6 -> 5.6)
...
def parse_duration_to_seconds(x):
    ...
    if m:
        h,m_,s_=map(int,m.groups()); return h*3600+m_*60+s_
    m=re.match(r"^(\d+):(\d{2})$", s)
    if m:
        m_,s_=map(int,m.groups()); return m_*60+s_
    try: return float(s)
    except: return np.nan
...
# Надёжное чтение и хэш одного uploaded-файла (без «съедания» буфера)
...
if "groups" not in st.session_state: st.session_state["groups"] = []  # [{name, files:[{name,hash,df,meta}]}]
...
def kpis_for_group(group):
    total_impr=0.0; total_views=0.0; ctr_vals=[]; avd_vals=[]
...
if nav.endswith("Dashboard"):
        ...
        if st.button("Добавить группу"):
            if not group_name.strip():
                ...
                for uf in files:
                    ...
                    if pack["hash"] in known: skipped += 1; continue
                    if pack["df"] is None or pack["df"].empty: continue
                    new_files.append(pack); known.add(pack["hash"])
                if new_files:
                    st.session_state["groups"].append({"name": group_name.strip(), "files": new_files})
                    st.success(f"Группа добавлена. Пропущено дублей: {skipped}.")
                    st.rerun()
                else:
                    st.error("Ни одного нового файла не добавлено (возможно, все дубликаты/пустые).")
    ...
    if not st.session_state["groups"]:
                ...
                if st.button("Добавить отчёты", key=f"append_btn_{gi}"):
                    if not add_more:
                        ...
                        for uf in add_more:
                            ...
                            if pack["hash"] in known: skipped += 1; continue
                            if pack["df"] is None or pack["df"].empty: continue
                            g["files"].append(pack); known.add(pack["hash"]); added += 1
                        if added:
                            st.success(f"Добавлено файлов: {added}. Пропущено дублей: {skipped}.")
                            st.rerun()
                        else:
                            st.info("Новые файлы не добавлены (дубликаты/пустые).")
        ...
        for gi, g in enumerate(st.session_state["groups"]):
            ...
            kpi_rows.append({
                "Группа": g["name"], "Показы": kp["impressions"], "Просмотры": kp["views"],
...
# ============== GROUP ANALYTICS (Year Mix) ==============
    ...
    if tool.startswith("Сравнение по годам"):
        ...
        if source_mode == "Группы из Dashboard":
            if not st.session_state["groups"]:
                st.info("Нет групп данных. Сначала добавьте их в Dashboard."); st.stop()
            names=[g["name"] for g in st.session_state["groups"]]
            selected = st.sidebar.multiselect("Выберите группы", names, default=names[:1])
            if not selected: st.info("Выберите хотя бы одну группу."); st.stop()
        ...
        # Источник: новые файлы (с опцией сохранить в группу)
            ...
            if not df_list: st.info("Загрузите хотя бы один CSV."); st.stop()
            ...
            if st.sidebar.checkbox("Сохранить эти файлы в группу"):
                ...
                if mode == "В существующую группу":
                    ...
                    if st.sidebar.button("Создать и сохранить"):
                        ...
                        for uf in up_files:
                            pack=load_uploaded_file(uf)
                            if pack["hash"] in known: continue
                            if pack["df"] is None or pack["df"].empty: continue
                            new_files.append(pack); known.add(pack["hash"])
                        if new_files:
        ...
        C = detect_columns(df)
        pub_col=C["publish_time"]; views_col=C["views"]
        missing=[]
        ...
        if missing: st.error("Не хватает колонок: " + ", ".join(missing)); st.stop()
        ...
        views_year = (df.groupby("_year", as_index=False)["_views_num"].sum()
                        .rename(columns={"_year":"Год","_views_num":"Суммарное количество просмотров"})
                        .sort_values("Год"))
        count_year = (df.groupby("_year", as_index=False).size()
                        .rename(columns={"_year":"Год","size":"Количество видео"})
                        .sort_values("Год"))

        if views_year.empty or count_year.empty:
            st.info("Недостаточно данных для построения графиков по годам."); st.stop()
        ...
        if ranking_years: parts.append("Лидируют по просмотрам: **" + " → ".join(ranking_years) + "**.")
        ...
        if len(frame)>=2:
            vals=[vy[y] for y in frame]; mx=max(vals); mn=min(vals)
            if mx>0 and (mx-mn)/mx <= 0.15: parts.append("В **2022–2024** суммарные просмотры держались примерно на одном уровне (±15%).")
        if prev_year and not any(pd.isna(x) for x in [views_ref,views_prev,cnt_ref,cnt_prev]):
            if close_enough(views_ref, views_prev, 0.12) and cnt_ref>cnt_prev:
                times = cnt_ref/max(cnt_prev,1); parts.append(f"При близких просмотрах у {prev_year} и {ref_year} "
                                                              f"в {ref_year}-м понадобилось больше видео (≈×{times:.1f}).")
